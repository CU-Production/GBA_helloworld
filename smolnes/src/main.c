...
// uint16_t frame_buffer[61440]; // 256x240 pixel frame buffer. Top and bottom 8 rows are not drawn.
// offset 64kb
#define frame_buffer     ((u16*)(EWRAM+0x10000))

// uint8_t rombuf[256 * 1024];   // Buffer to read ROM file into
// offset 192kb frame_buffer need 120kb
#define rombuf           ((u8*)(EWRAM + 0x30000))
...
uint8_t mem(uint8_t lo, uint8_t hi, uint8_t val, uint8_t write) {
  ...
  case 4:
    ...
    tmp = 0;
    {
      ...
      if ( keys_pressed & KEY_A )      tmp |= 0b00000001;
      if ( keys_pressed & KEY_B )      tmp |= 0b00000010;
      if ( keys_pressed & KEY_SELECT ) tmp |= 0b00000100;
      if ( keys_pressed & KEY_START )  tmp |= 0b00001000;
      if ( keys_pressed & KEY_UP )     tmp |= 0b00010000;
      if ( keys_pressed & KEY_DOWN )   tmp |= 0b00100000;
      if ( keys_pressed & KEY_LEFT )   tmp |= 0b01000000;
      if ( keys_pressed & KEY_RIGHT )  tmp |= 0b10000000;
    }
    if (lo == 22) {
      if (write) {
        keys = tmp;
...
int main(int argc, char **argv) {
  ...
  int max_filesize = 256*1024;
  if (mario_nes_size < max_filesize) max_filesize = mario_nes_size;
  for (int i = 0; i < max_filesize; i++) {
    u8 testdata = mario_nes[i];
    rombuf[i] = mario_nes[i];
  }
        ...
        if (scany == 241) {
          ...
          for (int cy = 0; cy < SCREEN_HEIGHT; cy++) {
            for (int cx = 0; cx < SCREEN_WIDTH; cx++) {
              int c_nes_id = (cy + OFFSET_H) * NES_WIDTH + cx + OFFSET_W;
              uint16_t c16 = frame_buffer[c_nes_id];
              uint8_t r5 = ((((c16 >> 11) & 0x1F) * 527) + 23) >> 6;
              uint8_t g6 = ((((c16 >> 5) & 0x3F) * 259) + 33) >> 6;
              uint8_t b5 = (((c16 & 0x1F) * 527) + 23) >> 6;
              m3_plot(cx, cy, RGB15(b5/8, g6/8, r5/8));
            }
          }
        }
